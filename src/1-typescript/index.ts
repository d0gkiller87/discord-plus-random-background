  ...
  return class extends Plugin {
    ...
    GetRandomImageURL( prevent_cache = false ) {
      ...
      while ( this.settings.image_urls_pool ) {
        ...
        if (
          this.image_urls.includes( image_url ) &&
    ...
    GetResizeImageURL( image_url: string ) {
     ...
     */
    ApplyBackgroundImage( should_change_image: boolean = true ) {
      let image_url: string;
     ...
     */
    ApplyRandomBackgroundImage( apply_immediately: boolean = true ) {
      if ( this.image_urls.length <= 0 ) return
      ...
      if ( apply_immediately ) this.ApplyBackgroundImage()
      this.PreloadImage()
    }
     ...
     */
    ClearInterval() {
      if ( this.settings.interval_id ) clearInterval( this.settings.interval_id )
    ...
    onStart() {
      ...
      this.settings = { ...this.default_settings, ...BdApi.Data.load( this.name, 'settings' ) }
      ...
      if ( !this.global_css ) {
        this.global_css = document.getElementById( global_css_id ) || Object.assign(
      ...
      if ( !this.preload_img ) {
        this.preload_img = document.getElementById( preload_img_id ) || Object.assign(
     ...
     */
    getSettingsPanel() {
      const interval_textbox = new Settings.Textbox(
        'Interval of image changes',
        'Time before switching to a new image (in seconds, <=0: do not switch images until a plugin/Discord restart)',
      ...
      const use_proxy_switch = new Settings.Switch(
        'Enable image proxy server', 'Use a proxy server to resize and crop images to window size which prevents the resizing animation while fading in',
        this.settings.use_image_proxy,
        new_data => {
      ...
      const background_transition_type = new Settings.Dropdown(
        'Background switch method', 'Methods of switching background images',
        this.settings.transition_type, [
      ...
      const opacity_textbox = new Settings.Textbox(
        '', '',
        this.settings.alpha,
        new_data => {
      ...
      const opacity_slider = new Settings.Slider(
      ...
      const fade_time_textbox = new Settings.Textbox(
        'Fading time of background image (in seconds)', '',
        this.settings.fading_seconds,
        new_data => {
      ...
      image_urls_textarea.addEventListener(
